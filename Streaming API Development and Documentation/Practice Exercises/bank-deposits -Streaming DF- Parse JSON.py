...
bankDepositsrawstreamDF = spark.readStream.format("kafka").option("kafka.bootstrap.servers","localhost:9092").option("subscribe","bank-deposits").option("startingOffsets", "earliest").load()                      

#TO-DO: using a select expression on the streaming dataframe, cast the key and the value columns from kafka as strings, and then select them
bankDepositsstreamDF = bankDepositsrawstreamDF.selectExpr("CAST(key as string) key", "CAST(value as string) value")
#TO-DO: using the kafka message StructType, deserialize the JSON from the streaming dataframe 
bankDepositsstreamDF.withColumn("value",from_json("value", kafkaMessageSchema)).select(col('value.*')).createOrReplaceTempView("BankDeposits")
# TO-DO: create a temporary streaming view called "BankDeposits" 
# it can later be queried with spark.sql

#TO-DO: using spark.sql, select * from BankDeposits
bankDepositsstreamStarDF =spark.sql("select * from BankDeposits")
...
bankDepositsstreamStarDF.writeStream.outputMode("append").format("console").start().awaitTermination()
